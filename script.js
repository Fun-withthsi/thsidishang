...
function startBootSequence() {
    if (!bootScreen || !bootLog || !bootFinal) {
        document.body.classList.remove('is-booting');
        return;
    }

    const bootSteps = [
        { label: '[ OK ]', message: 'kernel modules initialized', kind: 'ok' },
        { label: '[ OK ]', message: 'network interface online', kind: 'ok' },
        { label: '[ OK ]', message: 'theme profile loaded', kind: 'ok' },
        { label: '[ OK ]', message: 'portfolio sections indexed', kind: 'ok' },
        { label: '[ OK ]', message: 'navigation routes verified', kind: 'ok' },
        { label: '[ .. ]', message: 'finalizing launch', kind: 'muted' }
    ];
    ...
    const stepTicker = window.setInterval(() => {
        ...
        line.innerHTML = `<span class="${step.kind}">${step.label}</span> ${step.message}`;
